"""
MAVLink Parser for PX4 Telemetry - THREAD-SAFE VERSION
...
FIXES:
1. Added threading.Lock for all shared data structures
2. Fixed bounded buffer for latencies using collections.deque
3. Thread-safe counter operations
4. Atomic telemetry snapshots
5. Proper synchronization for metrics calculation
"""
...
class DroneConnection:
    """Manages connection to a single PX4 instance and parses telemetry - THREAD-SAFE"""
    ...
    def __init__(self, drone_id: int, name: str, host: str, port: int, color: str):
        ...
        # FIX: Use deque with maxlen for bounded buffer (thread-safer)
    ...
    def connect(self):
        ...
        try:
            ...
            # Use 'udp:' (client mode) to connect to PX4's MAVLink stream
            # This is the correct mode for connecting to PX4 SITL instances
        ...
        while self.running:
            try:
                ...
                if msg_type in DEMO_MESSAGES:
                    ...
                    # FIX: Update last_update with lock
                    with self._telemetry_lock:
    ...
    def _parse_message(self, msg):
        ...
        # FIX: Build update dict first, then apply atomically
        update = {}
        ...
        # FIX: Apply all updates atomically
        if update:
            with self._telemetry_lock:
    ...
    def get_telemetry(self) -> Dict[str, Any]:
        ...
        # FIX: Deep copy with lock to prevent race conditions
        with self._telemetry_lock:
            return deepcopy(self.telemetry)

    def get_metrics(self) -> Dict[str, Any]:
        """Calculate and return network metrics - THREAD-SAFE"""
        import statistics

        current_time = time.time()

        # FIX: Acquire locks in consistent order to prevent deadlocks
        with self._counts_lock:
            # Snapshot counts
            ...
            message_types = dict(self.message_type_counts)  # Copy

        with self._metrics_lock:
            # Snapshot timing data
            window_start = self.throughput_window_start
            conn_start = self.connection_start_time
            latencies_copy = list(self.latencies)  # Copy for safe iteration
        ...
        # FIX: Reset window with proper locking
        if window_duration >= 1.0:
            with self._counts_lock:
                # Re-check condition inside lock
                current_time_locked = time.time()
                if current_time_locked - window_start >= 1.0:
                    with self._metrics_lock:
                        self.throughput_window_start = current_time_locked
                    self.throughput_total_packets = 0
                    self.throughput_demo_packets = 0
        ...
        # FIX: Calculate latency stats from copied data
